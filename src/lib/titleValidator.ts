...
// 促销词 - 仅检测，不静默删除
const PROMOTIONAL_WORDS = ["best", "amazing", "top rated", "#1", "number 1", "no.1", "no 1", "best seller", "bestseller", "top-rated", "perfect", "incredible", "unbeatable", "must-have", "must have"];
...
export function cleanTitleDeterministic(title: string): string {
  ...
  // 规范无意义特殊字符：!!! -> 去除, ### -> 去除, ?? -> ?
  cleaned = cleaned.replace(/!{2,}/g, "").replace(/#{2,}/g, "").replace(/\?{2,}/g, "?");
  cleaned = cleaned.replace(/★|☆|♥|♦|◆|●|■|✔|✘/g, "");
