...
import { validateTitle, cleanTitleDeterministic, getTitleDisplay, validateHighlights, cleanHighlightsDeterministic, getHighlightsDisplay, validateTitleMobile, toTitleCaseGerman } from "./lib/titleValidator";
          ...
          for (let attempt = 0; attempt < 2; attempt++) {
            try {
              ...
              const compressed = await (async () => {
                const prompt = `Compress this Amazon ${platform === "amazon-de" ? "DE" : "US"} title to <=75 characters including spaces, keep natural ${platform === "amazon-de" ? "German" : "Title Case"}, no ALL CAPS, no markdown, no emoji, no promotional words, front-load brand within 60 chars: "${lastTitle}" Product: "${productName.trim()}" Facts: "${normalizedPoints}" Platform: ${platformLabel}. Output ONLY the title.`;
