...
中文核心卖点：<Product_Facts>${normalizedPoints}</Product_Facts>`;
  ...
  const handleGenerateVariants = async () => {
    ...
    setIsGenerating(true);
    try {
      const variantPrompt = `基于以下已生成的标题和亮点，再生成2套差异化变体，策略B-痛点解决风、策略C-感官营销风，保持原有卖点：当前标题：${result.title} 当前亮点：${result.highlights} 原始卖点：${sellingPoints} 平台：${platform}。以JSON {"variants":[{"strategy":"痛点解决风","title":"...","highlights":"..."},{"strategy":"感官营销风","title":"...","highlights":"..."}]} 输出`;
      const controller = new AbortController();
      ...
      if (m) {
        ...
        if (j.variants && Array.isArray(j.variants)) {
          ...
          const cur: Variant = { title: result.title, highlights: result.highlights, strategy: "极简参数风" };
